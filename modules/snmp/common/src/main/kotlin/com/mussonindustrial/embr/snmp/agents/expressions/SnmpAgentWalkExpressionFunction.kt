...
class SnmpAgentWalkExpressionFunction(
    val method: SnmpScriptMethod<List<QualifiedOidValue>>,
    val executor: SnmpScriptMethodExecutor,
) : AbstractFunction() {

    companion object {
        const val NAME = "snmpWalk"
    ...
    override fun execute(expressions: Array<out Expression>): QualifiedValue {
        ...
        return BasicQualifiedValue(result.map { it.value }.toTypedArray())
    }
}
